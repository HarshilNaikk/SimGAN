...
def run(
    ...
    f = open('drone_expert_data/trajs/{}.csv'.format(RUN_NUM), 'w')
    writer = csv.writer(f)
    ...
    H = .1
    H_STEP = np.random.random()*0.1
    R = np.random.random()*2
    INIT_XYZS = np.array([[R*np.cos((i/6)*2*np.pi+np.pi/2)*np.random.random(), (R*np.sin((i/6)*2*np.pi+np.pi/2)-R)*np.random.random(), H+i*H_STEP] for i in range(num_drones)])
    INIT_RPYS = np.array([[0, 0,  0.1*np.random.random()] for i in range(num_drones)])
    ...
    for i in range(NUM_WP):
        TARGET_POS[i, :] = R*np.cos((i/NUM_WP)*(2*np.pi)+np.pi/2)+INIT_XYZS[0, 0] + 0.1*np.random.random(), R*np.sin((i/NUM_WP)*(2*np.pi)+np.pi/2)-R+INIT_XYZS[0, 1]+0.1*np.random.random(), 0.01*np.random.randn()
...
if __name__ == "__main__":
    ...
    for k in range(NUM_RUNS):
        ...
        # parser.add_argument('--runnum',             default=k, type=int, metavar='')
